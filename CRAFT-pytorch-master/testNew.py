...
parser.add_argument('--test', default=True, action='store_true', help='if True saves computation statistics')
...
if __name__ == '__main__':
    ...
    for k, image_path in enumerate(image_list):
        ...
        print("delta time : {}s".format(deltaTime))

        # save score text
        filename, file_ext = os.path.splitext(os.path.basename(image_path))
        real_folder = result_folder + '/' + image_path.replace('images', '').replace(filename + file_ext, '')
        mask_file = real_folder + "/res_" + filename + '_mask.jpg'
        # region_file = real_folder + "/res_" + filename + 'region.jpg'

        cv2.imwrite(mask_file, score_text)
        # cv2.imwrite(region_file, score_text[0:score_text.shape[0], 0:int(score_text.shape[1]/2)])
        ...
        if(args.test):
            ...
            for i in range(len(polys)):
                tnew = time.time()
                incorrect, correct = textOCR.getString(image_path, polys[i])
                curImg["BBs"][i] = {
                    "BB" : polys[i].tolist(),
                    "strings" : incorrect,
