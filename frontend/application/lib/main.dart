...
class _HomePageState extends State<HomePage> {
  ...
  LinkedHashMap grid = new LinkedHashMap<String, List<String>>();
  ...
  void updateText(
    ...
    /*grid["Top Left"] = List<String>.from(["ciao", "gigi", "gigi", "gigi"]);
    grid["Top"] = List<String>.from(["ciao2", "gigi"]);
    grid["Top Right"] = List<String>.from(["ciao3", "gigi"]);
    grid["Center Left"] = List<String>.from(["ciao4", "gigi", "gigi"]);
    grid["Center"] = List<String>.from(["ciao5", "gigi"]);
    grid["Center Right"] = List<String>.from(["ciao6", "gigi"]);
    grid["Bottom Left"] = List<String>.from(["cia7o", "gigi"]);
    grid["Bottom"] = List<String>.from(["ciao8", "gigi"]);
    grid["Bottom Right"] = List<String>.from(["ciao9", "gigi"]);*/
    list = [];
    grid.entries.forEach((e) => {
          list.add(new GridElementRound(
            position: e.key,
                  ...
                  children: <Widget>[
                    RawMaterialButton(
                      onPressed: () {
                        ...
                        showDialog(
                            ...
                            }).then((value) =>setState(() {print(value); url = value; print(url);}));
                    ...
                    RawMaterialButton(
                      onPressed: () async {
                        String path;
                        path = await shotPic();
                        final resp = await uploadImage(path);
                        if (resp.statusCode == 200) {
                          final Map parsed = json.decode(resp.body);
                          Scaffold.of(context).showSnackBar(
                              snackBarText(parsed["threeWords"] as List));
                          setState(() {
                            imgPath = path;
                            updateText(
                                parsed["grid"] as Map,
